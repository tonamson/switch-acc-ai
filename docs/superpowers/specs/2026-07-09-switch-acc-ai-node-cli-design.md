...
Port the existing `cx` Bash CLI into a public npm package named `switch-acc-ai` with a single executable command, `sacc`.

The new CLI keeps the selected feature set and account storage model, but moves the implementation to TypeScript so the interactive experience can use a richer command-palette style UI, cleaner status/list output, and maintainable modules.

## Goals

- Publish a public npm package named `switch-acc-ai`.
- Expose only the `sacc` command. Do not expose `cx` as a compatibility alias.
- Support `npx switch-acc-ai` and global install usage with `sacc`.
...
- Do not add new account-switching features beyond the selected `cx` surface.
...
The package will use npm's `bin` field to map the command name `sacc` to the built executable, for example:
...
The Node CLI will preserve the selected command surface with `sacc` replacing `cx`:
...
sacc remove <name>            delete an account profile after confirmation
```
...
The Node CLI will keep the selected file layout:

```text
CODEX_ACCOUNTS_DIR or ~/.codex-accounts
CODEX_SHARED_HOME or ~/.codex
~/.codex-accounts/.selected
~/.codex-accounts/<name>
```
...
- `src/bin/sacc.ts`: executable entrypoint, top-level error boundary.
- `src/cli/commands.ts`: command definitions and argument routing.
- `src/core/config.ts`: resolve environment-driven paths and fallbacks.
- `src/core/accounts.ts`: validate names, list profiles, read/write selected account, rename/remove accounts, link shared assets.
...
- Keep text concise.
- Use ASCII-safe labels and separators unless configured otherwise.
- Use clear sections for help output.
- Use compact tables or compact blocks for account list and status.
- Make the selected account visually obvious.
...
```text
Switch Account AI  command palette
context  codex    selected  acc2

> 01  Run with account
  02  Login account
  03  Set active account
...
- Account not found: suggest `sacc login <name>` or `sacc list`.
- Missing selected account: suggest `sacc use <name>` or `sacc pick`.
...
- `sacc remove <name>`: require typed confirmation of the exact account name.

For `sacc status --all`, failures must be isolated per account. The command should render successful accounts and per-account error rows, then exit non-zero if any account failed.
...
- Unit tests for account listing, selected account read/write, rename, remove, and shared asset linking.
...
- Smoke tests against the built executable `dist/bin/sacc.js`.
...
The existing Bash `cx` script may remain in the repository during the port, but it is not part of the npm package command surface. The package should publish only `sacc`.

The README should be updated after implementation to document:

- `npx switch-acc-ai`
- global install usage
- command mapping from `cx` examples to `sacc`
